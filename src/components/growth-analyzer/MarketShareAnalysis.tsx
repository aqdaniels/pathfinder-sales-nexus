...
import { Info, ArrowUp, ArrowDown, Coins } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Badge } from "@/components/ui/badge";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { DealSizeSegmentation } from "./DealSizeSegmentation";
...
export const MarketShareAnalysis = ({ client, industry, timeframe }: MarketShareAnalysisProps) => {
  ...
  const [analysisView, setAnalysisView] = useState("marketPosition");
    ...
    <div className="space-y-4">
      <Tabs value={analysisView} onValueChange={setAnalysisView} className="w-full">
        <TabsList className="inline-flex mb-4">
          <TabsTrigger value="marketPosition" className="flex items-center gap-1">
            Market Position
          </TabsTrigger>
          <TabsTrigger value="dealSize" className="flex items-center gap-1">
            <Coins className="h-4 w-4" />
            Deal Size Analysis
          </TabsTrigger>
        </TabsList>
        
        <TabsContent value="marketPosition" className="space-y-4">
          <Card>
            <CardHeader>
        ...
        <TabsContent value="dealSize">
          <DealSizeSegmentation client={client} industry={industry} />
        </TabsContent>
      </Tabs>
    </div>
  );
